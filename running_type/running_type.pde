 ...
 // Otso Sorvettula https://www.openprocessing.org
...
// Declaring sound files that will be used 
SoundFile sound;
SoundFile distortion; 
...
// Variables to check if the noise level is too high
float level;
boolean tooNoisy = false;

// Variables to be used when generating waves in the background
float bgOpacity = 250;
float xstep = 20;
float ystep = 5;
float y = 0;
...
void setup() {
  ...
  // Set up audio player to narrate the play
  sound = new SoundFile(this, "NotI.mp3");
  sound.play();
  sound.rate(.5);
  // Distortion for when the narrator is interrupted
  distortion = new SoundFile(this, "distort_04.wav");
...
void draw() { 
  // background opacity will change depending on the current state to create blurry effect
  fill(0,bgOpacity);
  ...
  level = mic.mix.level();
  // and if it is, the distortion gets triggered, waves in the background change shape, bg opacity becomes lower to create blurry effect
  if (level >= 0.05) {
    tooNoisy = true;
    waveChange = .02;
    bgOpacity = 70;
  ...
  // when the noise level gets back to normal, so do all impacted parameters
  else {
    tooNoisy = false;
    waveChange = .08;
    bgOpacity = 255;
  ...
  for (int i = 0; i < letters.length; i++) {
   ...
   // Display all letters
   letters[i].display();
      // Noise also makes individual letters scatter
      // If not, they return to their original location
      if (tooNoisy && letters[i].x + (textWidth(message.charAt(i))) > 0 && letters[i].x - (textWidth(message.charAt(i))) < width) {
        letters[i].shake();
      } else {
        letters[i].home();
    }
...
// Same behaviour may be achieved by pressing any key on the keyboard
// When the key is released default behaviour restores 
void keyPressed() {
  //waveSize = 80;
  waveChange = .02;
  bgOpacity = 70;
